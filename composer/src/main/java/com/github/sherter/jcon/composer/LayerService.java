...
public class LayerService {
  ...
  public int createThrottleLayer() throws IOException {
    int id = counter.incrementAndGet();
    ThrottleLayer layer = new ThrottleLayer(this);
    layers.put(id, layer);
    return id;
  }
  ...
  public int createShortestPathCalculaterLayer(JsonNode config) throws IOException {
    ...
    ServerSocketChannel listenSocket = reactor.listen(null, forwarder.callbackFactoryForNewSwitchConnections(),
        forwarder::acceptConnectionFromSwitch);

    Thread thread = new Thread(() -> reactor.loop());
    thread.start();
    Layer layer = new InterceptableForwarderLayer(forwarder, "shortest_path_calculator",
